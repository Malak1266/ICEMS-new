...
# ─── CONSTANTES ────────────────────────────────────────────────────────────────
# Nouveau paradigme : 1 rigid body générique avec 4 sphères (post-réunion 2026-05).
# Les anciens noms d'instruments restent supportés (rétro-compat) mais pointent
# tous vers 4 sphères, conformément au matériel réel (corps en fibre de carbone).
N_SPHERES   = {"generic": 4, "bipolar": 4, "scissors": 4, "aspirator": 4}

# Paramètres temporels (fréquence d'acquisition Atracsys)
DEFAULT_FS           = 14.14   # Hz — fréquence des 3 anciennes sessions (~12-14 Hz)
DEFAULT_MAX_VELOCITY = 700.0   # mm/s — vitesse max physiquement plausible d'un geste

def compute_max_jump(fs: float, max_velocity: float = DEFAULT_MAX_VELOCITY) -> float:
    """Saut maximal entre 2 frames consécutives à la fréquence fs.

    À 14 Hz : 700/14 = 50 mm (valeur historique).
    À 60 Hz : 700/60 ≈ 11.7 mm.
    À 120 Hz : 700/120 ≈ 5.8 mm.
    """
    return max_velocity / fs

MAX_JUMP_MM = compute_max_jump(DEFAULT_FS)   # fallback historique (≈ 50 mm)
...
def track_sequence(csv_clean_path: Path, n_spheres: int,
    ...
    Args:
        csv_clean_path : chemin du fichier fiducials_clean.csv
        n_spheres      : nombre de sphères du rigid body (4 dans le nouveau setup)
        max_jump       : saut maximal autorisé entre 2 frames (mm).
                         À calculer via compute_max_jump(fs) selon la fréquence réelle.
    ...
    for fi in frames_sorted:
        ...
        assignment = hungarian_assign(prev_xyz, curr_xyz, max_jump=max_jump)
...
def compute_features(df_tracks: pd.DataFrame, fs: float = DEFAULT_FS):
    """
    Calcule les 6 features SANS pos_mag (invariants à la pose caméra).

    Args:
        df_tracks : sortie de track_sequence()
        fs        : fréquence d'acquisition réelle (Hz). À 60 Hz, fournir fs=60,
                    sinon les unités mm/s, mm/s², mm/s³ sont fausses.
    ...
    Retourne : (array [T, 6], list frames)
    """
    dt = 1.0 / fs
...
def process_one(seq_path: str, out_dir: str = "pipeline_output",
    ...
    instrument = seq_path.parts[-3]    # ex: "bipolar" ou "generic"
    n_sph = N_SPHERES.get(instrument, 4)
    max_jump = compute_max_jump(fs)

    print(f"\n{'='*60}")
    print(f"Séquence : {seq_path}")
    print(f"Instrument : {instrument}  ({n_sph} sphères attendues)")
    print(f"Fréquence  : {fs:.2f} Hz  →  max_jump = {max_jump:.1f} mm/frame")
    ...
    df_tracks, stats = track_sequence(csv_path, n_sph, max_jump=max_jump)
    ...
    features, _ = compute_features(df_tracks, fs=fs)
...
def process_all(dataset_path: str, out_dir: str = "pipeline_output",
    ...
    # "generic" couvre les nouvelles collectes 1-rigid-body ;
    # les anciens noms sont conservés pour rétro-compat avec les sessions <= 2026-05.
    for instrument in ["generic", "bipolar", "scissors", "aspirator"]:
        for motion in sorted({p.name for p in (base / instrument).iterdir()
                              if p.is_dir()}) if (base / instrument).exists() else []:
            seq_dir = base / instrument / motion
            for rep in sorted(seq_dir.iterdir()):
                if rep.is_dir() and rep.name.startswith("R"):
                    _, stats = process_one(str(rep), out_dir, fs=fs)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--fs",     type=float, default=DEFAULT_FS,
                        help=f"Fréquence d'acquisition Atracsys en Hz "
                             f"(défaut {DEFAULT_FS} = anciennes sessions ; "
                             f"60-120 recommandé pour les nouvelles)")
    args = parser.parse_args()

    if args.seq:
        process_one(args.seq, args.out, fs=args.fs)
    elif args.all:
        process_all(args.all, args.out, fs=args.fs)
    else:
